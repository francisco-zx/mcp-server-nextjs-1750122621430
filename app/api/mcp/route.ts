...
function getZodTypeFromOpenApiSchema(schema, z) {
  ...
  switch (type) {
    ...
    case "boolean":
      return z.boolean();
    case "array":
      // Basic array of strings, can be enhanced if item schema is known
    ...
    case "object":
      ...
      return z.record(z.string(), z.any()); // Fallback for generic object
    default:
      return z.string();
  }
}
...
function getZodTypeFromOpenApiParam(param, z) {
  if (!param.schema) return z.string(); // Default if no schema
...
export { GET, PUT, PATCH, DELETE, OPTIONS, HEAD };